...
async fn test_optimized_metrics_response() {
    ...
    assert!(
        !meta.contains_key("resp_version"),
        "resp_version should be removed"
    );
    ...
    if !data.is_empty() {
        ...
        assert!(
            !first_series.as_object().unwrap().contains_key("expression"),
        ...
        assert!(
            ...
            "query_index should be removed"
        );
        assert!(
            !first_series.as_object().unwrap().contains_key("length"),
            "length should be removed"
        );
        assert!(
            !first_series.as_object().unwrap().contains_key("start"),
            "start should be removed"
        );
        assert!(
            !first_series.as_object().unwrap().contains_key("end"),
    ...
    if !data.is_empty() {
        println!(
            "Series keys: {:?}",
            data[0].as_object().unwrap().keys().collect::<Vec<_>>()
        );
    }
}
