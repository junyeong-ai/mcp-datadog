...
impl ServicesHandler {
    pub async fn list(
        ...
        let response = client.get_service_catalog(
            page_size_param,
            page_number_param,
            filter_env.clone(),
        ).await?;
        ...
        let data = json!(response.data.iter().map(|service| {
            ...
            if let Some(attributes) = &service.attributes {
                ...
                if let Some(contacts) = &attributes.contacts {
                    formatted_service["contacts"] = json!(contacts.iter().map(|c| json!({
                        "name": c.name,
                        "email": c.email,
                        "type": c.contact_type
                    })).collect::<Vec<_>>());
                ...
                if let Some(links) = &attributes.links {
                    formatted_service["links"] = json!(links.iter().map(|l| json!({
                        "name": l.name,
                        "url": l.url,
                        "type": l.link_type
                    })).collect::<Vec<_>>());
                ...
                if let Some(repos) = &attributes.repos {
                    formatted_service["repos"] = json!(repos.iter().map(|r| json!({
                        "name": r.name,
                        "url": r.url,
                        "provider": r.provider
                    })).collect::<Vec<_>>());
                ...
                if let Some(docs) = &attributes.docs {
                    formatted_service["docs"] = json!(docs.iter().map(|d| json!({
                        "name": d.name,
                        "url": d.url,
                        "provider": d.provider
                    })).collect::<Vec<_>>());
